...
const cardNumberPattern = {
  ...
  dispatch: function (appended, dynamicMasked) {
    ...
    const founderMask = dynamicMasked.compiledMasks.find(function (item) {
...
securityCodeMasked.on("accept", () => {
  upadteSecuryCode(securityCodeMasked.value)
})

function upadteSecuryCode(number1) {
  const ccSecutiry = document.querySelector(".cc-security .value")

  ccSecutiry.innerText = number1.length === 0 ? "123" : number1
}
...
cardMasked.on("accept", () => {
  ...
  updatecardnumber(cardMasked.value)
})
...
function updateExpirationDate(date) {
  ...
  ccExpiration.innerText = date.length === 0 ? "02/12" : date
}
